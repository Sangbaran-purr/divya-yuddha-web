...
(async () => {
  ...
  // ── G4 · tap CANCEL TABLE on the ghost plaque ──────────────────────────────
  console.log("\n── G4 · CANCEL TABLE on the ghost ──");
  const before = lobby.size(), logBefore = logLines.length;
  const btn = two.w.document.querySelector('[data-act="cancel"]');
  console.log("    plaque offers CANCEL TABLE: " + !!btn);
  if (btn) {
    btn.click(); await H.sleep(400);
    const doBtn = two.w.document.querySelector("[data-cancel-do]") || two.w.document.querySelector(".hall-act");
    console.log("    cancel sheet act: " + (doBtn ? doBtn.textContent.trim() : "(none)"));
    if (doBtn) doBtn.click();
    await H.sleep(4000);
  }
  const cer = st2().ceremony;
  console.log("    ceremony after the tap: " + JSON.stringify(cer));
  console.log("    lobby size before=" + before + " after=" + lobby.size() + "  (ghost still listed: " + lobby.list().some((t)=>t.id===ghost.id) + ")");
  console.log("    server received a close? " + (logLines.length > logBefore ? logLines.slice(logBefore).join(" | ") : "NO new server log lines"));
  const mm2 = await new ethers.Contract(escAddr, ESC, provider).matches(BigInt(t0.escrowMatchId));
  console.log("    escrow state unchanged: " + Number(mm2.state) + " (2=MATCHED — no refund, no abort)");
