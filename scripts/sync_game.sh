...
# twenty links, fail loudly on drift. Handles re-runs — an existing ?v=<oldsha> is rewritten to the new sha.
...
# (S-SYNC-GUARD-1: the house-nav mobile dropdown duplicated "The Game" on six pages [rite/index/treasury/store/
#  explore/mint each +1 -> 16], and dashboard.html + register.html joined via S-DASH-NAV-1 / S-REG-NAV [+2 each].
#  Managed set now 9 pages / 20 links — every gated-game linker gets its stamp bound to the game bytes.)
ENTRY_FILES="$SITE_REPO/rite.html $SITE_REPO/index.html $SITE_REPO/treasury.html $SITE_REPO/demo/index.html $SITE_REPO/store.html $SITE_REPO/explore.html $SITE_REPO/mint.html $SITE_REPO/dashboard.html $SITE_REPO/register.html"
LINKS_BEFORE="$(grep -oF 'game/index.html' $ENTRY_FILES | wc -l | tr -d ' ')"
if [ "$LINKS_BEFORE" != "20" ]; then
  echo "error: expected exactly 20 site->game entry links (rite.html x4 + index.html x3 + treasury.html x2 + demo/index.html x1 + store.html x2 + explore.html x2 + mint.html x2 + dashboard.html x2 + register.html x2), found $LINKS_BEFORE (site nav changed)" >&2
  exit 1
fi
...
if [ "$LINKS_STAMPED" != "20" ]; then
  echo "error: entry-link stamping did not produce 20 stamped links (got $LINKS_STAMPED)" >&2
  exit 1
fi
